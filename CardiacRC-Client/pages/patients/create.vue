...
<div >
  ...
  <b-container>
    ...
    <form class="needs-validation" @submit.prevent="createPatient" :disabled="!isFormValid">

      <div class="col-sm-4 offset-sm-4">
        <b-form-group id="name" label="Name:" label-for="name" :invalid-feedback="invalidNameFeedback" :state="isNameValid">
          <b-input id="name" v-model.trim="name" placeholder="Enter your name" :state="isNameValid" trim></b-input>
        </b-form-group>
      </div>
        ...
        <div class="col-sm-4 offset-sm-4">
          <b-form-group id="username" label="Username:" label-for="username" :invalid-feedback="invalidUsernameFeedback" :state="isUsernameValid">
            <b-input id="username" v-model.trim="username" placeholder="Enter your username" :state="isUsernameValid" trim></b-input>
          </b-form-group>
        </div>
        ...
        <div class="col-sm-4 offset-sm-4">
          <b-form-group id="password" label="Password:" label-for="password" :invalid-feedback="invalidPasswordFeedback" :state="isPasswordValid">
            <b-input id="password" type="password" v-model.trim="password" placeholder="Enter your password" :state="isPasswordValid" trim></b-input>
          </b-form-group>
        </div>

        <div class="col-sm-4 offset-sm-4">
            <b-form-group id="email" label="Email:" label-for="email" :invalid-feedback="invalidEmailFeedback" :state="isEmailValid">
              <b-input ref="email" v-model.trim="email" type="email" :state="isEmailValid"  placeholder="Enter your e-mail" trim/>
            </b-form-group>
        </div>

        <div class="col-sm-4 offset-sm-4">
            <b-form-group id="healthNumber" label="Health Number:" label-for="healthNumber" :invalid-feedback="invalidHealthNumberFeedback" :state="isHealthNumberValid">
              <b-input id="healthNumber" type="number" v-model.trim="healthNumber" placeholder="Enter your health number" :state="isHealthNumberValid" trim></b-input>
            </b-form-group>
        </div>

        <div class="col-sm-4 offset-sm-4">
            <label>Associated Professional:</label>
            <b-select v-model="professionalUsername" :options="professionals" :state="isProfessionalValid" required value-field="username" text-field="name">
              <template v-slot:first>
                <option :value="null" disabled>-- Please select a Professional --</option>
              </template>
            </b-select>
        </div>
        ...
        <div class="col-sm-4 offset-sm-4">
          ...
          <button @click.prevent="createPatient" class="btn btn-secondary" :disabled="!isFormValid">Create</button>
        </div>
    </form>
  </b-container>
</div>
...
<script>
  ...
  created() {
    this.$axios.$get('/api/professionals')
      .then(professionals => {
  ...
  computed: {
    ...
    isProfessionalValid () {
        ...
        return this.professionals.some(professional => this.professionalUsername === professional.username)
    },
